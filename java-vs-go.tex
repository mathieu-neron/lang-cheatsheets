...
Go's stdlib is intentionally minimal here: \code{slice} and \code{map} cover
80\% of needs; the rest leans on \code{container/*} packages, channels, or
3rd-party libraries (e.g. \code{github.com/emirpasic/gods}, \code{google/btree},
\code{elliotchance/orderedmap}).
...
\toprule
\rowcolor{accentDark}
\refhead{\normalfont Java} & \refhead{Behavior} & \refhead{Go} \\
\midrule
ArrayList<T>        & dynamic array & \code{[]T} (slice) \\
LinkedList<T>       & doubly-linked & \code{container/list} \\
int[] / Object[]    & fixed-size array & \code{[N]T} (array, value type) \\
HashSet<T>          & unordered set & \code{map[T]struct\{\}} \\
LinkedHashSet<T>    & insertion-ordered set & slice + \code{map[T]int} idiom (or \code{orderedmap}) \\
TreeSet<T>          & sorted set & \code{google/btree} or \code{emirpasic/gods/sets/treeset} \\
EnumSet             & bit-set over enum & \code{map[Day]struct\{\}} or hand-rolled bitmask \\
HashMap<K,V>        & unordered map & \code{map[K]V} (RANGE ORDER IS RANDOMIZED) \\
LinkedHashMap<K,V>  & insertion-ordered & \code{elliotchance/orderedmap} (PyPI-style) \\
TreeMap<K,V>        & sorted map & \code{google/btree} or \code{emirpasic/gods/.../treemap} \\
IdentityHashMap     & identity-keyed & \code{map[unsafe.Pointer]V} (rare; usually a code-smell in Go) \\
EnumMap             & enum-keyed map & plain \code{map[Day]V} \\
WeakHashMap         & weak keys & no native; not idiomatic in Go \\
ArrayDeque (queue)  & FIFO & \code{container/list} or channel \\
ArrayDeque (stack)  & LIFO & slice with \code{append} / pop-by-reslice \\
ArrayDeque          & double-ended & \code{container/list} \\
PriorityQueue<T>    & min-heap & \code{container/heap} (impl interface on a slice) \\
ConcurrentHashMap   & thread-safe map & \code{sync.Map} (specific access patterns), or \code{map} + \code{sync.RWMutex} \\
BlockingQueue       & blocking FIFO & \code{chan T} (buffered or unbuffered) \\
DelayQueue          & expiry-ordered & \code{container/heap} + \code{time.Timer} \\
List.copyOf, Map.of & immutable views & no native; convention + lint, or freeze-on-clone \\
Stream / Collectors & lazy ops & \code{slices}, \code{maps} pkgs (1.21+); \code{iter} (1.23+) \\
\bottomrule
\end{tabularx}

\bigskip

\cheatsub{Slices, arrays, maps (the natives)}

\begin{construct}{Slice (dynamic) vs array (fixed)}
\noindent
\begin{minipage}[t]{0.485\linewidth}
\begin{lstlisting}[style=java, title=Java]
...
\begin{construct}{Slice gotcha (shared backing array)}
\noindent
\begin{minipage}[t]{0.485\linewidth}
\begin{lstlisting}[style=java, title=Java]
...
\begin{construct}{Map (HashMap analog) --- iteration is randomized}
\noindent
\begin{minipage}[t]{0.485\linewidth}
\begin{lstlisting}[style=java, title=Java]
...
\cheatsub{Sets (no native type --- use map keys)}
...
\cheatsub{Maps with order or sort guarantees}

\begin{construct}{LinkedHashMap (insertion order)}
\noindent
\begin{minipage}[t]{0.485\linewidth}
\begin{lstlisting}[style=java, title=Java]
...
\begin{construct}{TreeMap (sorted map)}
\noindent
\begin{minipage}[t]{0.485\linewidth}
\begin{lstlisting}[style=java, title=Java]
...
\begin{construct}{Queue (FIFO) and Deque}
\noindent
\begin{minipage}[t]{0.485\linewidth}
\begin{lstlisting}[style=java, title=Java]
...
\begin{construct}{Concurrent map and queue}
\noindent
\begin{minipage}[t]{0.485\linewidth}
\begin{lstlisting}[style=java, title=Java]
...
\begin{construct}{Counter / multiset, ring buffer, named tuple}
\noindent
\begin{minipage}[t]{0.485\linewidth}
\begin{lstlisting}[style=java, title=Java]
...
\begin{construct}{Mutability / read-only}
\noindent
\begin{minipage}[t]{\linewidth}
\small
\textbf{Java}: \code{ArrayList}, \code{HashMap}, \code{HashSet} mutable.
\code{List.of/Set.of/Map.of} return immutables. \code{Collections.unmodifiableX}
wraps a view.\par
\textbf{Go}: \code{slice}, \code{map}, struct fields all mutable. \emph{There is
no language-level immutability.} Convention: hand out copies (\code{slices.Clone},
\code{maps.Clone}); accept read-only intent via interface methods that don't
mutate, or pass a copy. Some libraries freeze-on-clone but it's not idiomatic.
\end{minipage}
\end{construct}
